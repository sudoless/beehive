...
// Parser is used to build a be.HandlerFunc that will populate the context.Context with the Values.
...
func Parser(fields []string) be.HandlerFunc {
	...
	return func(ctx context.Context, req *http.Request) be.Responder {
		...
		res := be.Next(context.WithValue(ctx, contextValuesKey, query), req)
		pool.Put(query)
